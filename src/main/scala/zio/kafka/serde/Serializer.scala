...
trait Serializer[-T] {
  ...
  def contramap[U](f: U => T): Serializer[U] =
    Serializer((topic, headers, u: U) => serialize(topic, headers, f(u)))

  /**
   * Create a serializer for a type U based on the serializer for type T and an effectful mapping function
   */
  def contramapM[R, U](f: U => RIO[R, T]): Serializer[U] =
    Serializer((topic, headers, u: U) => f(u).flatMap(serialize(topic, headers, _)))

  /**
   * Returns a new serializer that executes its serialization function on the blocking threadpool.
   */
  def blocking: Serializer[T] =
    Serializer((topic, headers, t: T) => zioBlocking(serialize(topic, headers, t)))
...
object Serializer extends Serdes {
   ...
   */
  def apply[T](ser: (String, Headers, T) => Task[Array[Byte]]): Serializer[T] =
    new Serializer[T] {
