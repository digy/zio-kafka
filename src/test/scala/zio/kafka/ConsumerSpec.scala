...
import zio.{ durationInt, Chunk, Clock, Promise, Ref, Schedule, Task, ZIO, ZLayer }
  ...
  override def spec: ZSpec[TestEnvironment, Throwable] =
    suite("Consumer Streaming")(
      ...
      test("should close old stream during rebalancing under load") {
        ...
        def checkAssignments(allAssignments: Atomic[Map[Int, List[Int]]])(instances: Set[Int]) =
          ZStream
        ...
        for {
          ...
          allAssignments = Ref.unsafeMake(Map.empty[Int, List[Int]])
      ...
      ((Kafka.embedded ++ ZLayer.environment[Clock] >>> producer) ++ Kafka.embedded)
