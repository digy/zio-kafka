...
object DeserializerSpec extends DefaultRunnableSpec {
  override def spec = suite("Deserializer")(
    suite("asOption")(
      ...
      test("deserialize to None when value is null also when underlying deserializer fails on null values") {
        val deserializer = Deserializer[Nothing]((_, _, _) => ZIO.fail(new RuntimeException("cannot handle null")))
  ...
  private lazy val stringDeserializer: Deserializer[String] = Serde.string
}
